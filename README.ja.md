...
## 6.5 ベンダ・ルーティングとバックエンド・ベンチ（gama）

§6 は *ティア*（どの大きさのモデルか）を選ぶ。それとは別の任意レイヤー ── **gama**
（蝦蟇）── が *ベンダ* を選ぶ。すなわち各タスク **クラス** をどのバックエンドに出すか。
フロンティア＋ローカルのモデル群を、各クラスごとに「実測で最良のベンダ」へ振り分け
ながら、ローカル主権レーンも保てる。

**バックエンド**（`backends.py`・すべて stdlib・使うまで不活性）: `claude-tui`（Claude
Code の TUI を `claude-cli-run.py` 経由で駆動＝定額サブスクのレーン。従量課金の
`claude --print`/Agent-SDK ではない）、`codex`（`codex exec`）、`gemini`（OpenAI互換・
キーでゲート・後付け）、`ollama`（ローカル HTTP、または `transport:"ssh"` でリモートの
`ollama run`）、`ssh-openai`（OpenAI互換サーバ ── **MLX `mlx_lm.server`**・LM Studio・vLLM
── をリモート箱で SSH 越しに呼ぶ。例: Mac Studio の強モデル floor）、加えて既存の
`claude-cli` / `echo` / `null`。

**`GamaBackend` が決定的な Conductor**: 呼び出しごとの `task_type`（executor が seam 越し
に渡す）を読み、`routing_table` のバックエンドへ振り分ける。未マップ/欠落は
`default_backend` にフォールバック。ルーティング経路に LLM はいない ── 表は *実測* で
あって推測ではない。

**`EnsembleBackend` は振り分けでなく"重ねる"**。gama が1タスクを1ベンダに送るのに対し、
`--backend ensemble` は **同一タスクに N モデルを走らせて統合する** ── モデル組み合わせ
ループを seam に乗せたもので、`tehai run`/`bench` から他の backend と同様に駆動できる。
戦略: `synthesize`（統合役が全候補から最終解を書く）/ `majority` / `first`。同じモデルを
`temperature` 付きで繰り返せば homogeneous な自己アンサンブル、別モデルを並べれば
heterogeneous な mixture-of-agents。tehai 上で単体と比較: `tehai bench --backends
ensemble,ssh-openai --config cfg.json`。`examples/ensemble.example.json` 参照。

**ベンチが外部錨**。`tehai bench` は、タスククラス別の小さなスイートを **決定的チェッカ**
（コード実行・数値完全一致・必須要素の有無 ── LLM judge は使わない）で各バックエンドに
かけ、`routing_table` を提案する（クラス毎にスコア最大・同点は latency→cost で決着）。
採用は calibrate と同じ作法 ── レビューして `--config` にマージし、`--backend gama` で実行:

```
# 無料の決定的スモーク（配線確認のみ）:
tehai bench --backends echo

# 実ベンダを実測し、表を提案（レート制限のためスイートは小さく）:
...
# 人手レビュー後、routing_table を cfg.json にマージし、gama で実行:
tehai run "build X" --backend gama --config cfg.json
```

リモートの **強モデル floor**（ポート非開放・プロンプトは stdin）: `ssh-openai` は
OpenAI互換サーバ（**MLX `mlx_lm.server`**・LM Studio・vLLM）を
`ssh <host> curl localhost:<port>/v1/…` で呼ぶ ── Mac Studio の MLX で実証済み
（0.5〜2 秒/コール）。ollama ホストには `ollama` レーンの `transport:"ssh"`（`ollama run`）。
`examples/gama_config.macstudio.example.json` 参照。

正直に: *文章* クラスは粗い近似（品質でなく形）で採点し、*コード* クラスは **モデル出力を
実行** する（opt-in、`--sandbox` と同様）。オーケストレーションは「クラス＋コストごとに
最良の専門家へ振る」ことで勝つ ── 単一タスクで最強モデルの実力を超えはしない。
