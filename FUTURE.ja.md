...
- **決着（公平トークン＋ルーティング修正の試行）**: 8192 トークンにすると 122B は c1 を回復
  （あれはトークン切れ）だが c3(roman) は依然 0＝**本物の取りこぼし**（32B コード特化が 122B 推論機に勝つ領域）。
  12問の最終: 軽量主権スタックと 122B は **0.92 で同点・穴は相補的**（軽量は r4 曜日を落とし、122B は c3 を落とす）。
  研究レーンに tool メンバーを足す修正は **r4 を救えなかった**（統合役が tool の正答を採らない）。
  正直な結論: **構造化軽量混合は 122B と互角（勝ちではない・前回の差は一部 122B のトークン切れ）・完全ローカル&主権**。
  再現: `python3 -m experiments.moa_vs_strong <config>`（`examples/moa_vs_strong.example.json`）。
